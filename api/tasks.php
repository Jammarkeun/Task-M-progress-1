...
switch ($method) {
    case 'GET':
        ...
        // Check if we want to fetch completed tasks
        if (isset($_GET['status']) && $_GET['status'] === 'completed') {
            $stmt = $task->getCompletedTasks(); // Assuming you have this method in your Task class
        } else {
            $stmt = $task->read();
        }
    ...
    case 'PUT':
        ...
        if (isset($data->id)) {
            ...
            $task->due_date = isset($data->due_date) ? $data->due_date : null; // Check if due_date is provided
            $task->status = isset($data->status) ? $data->status : 'pending'; // Default if not set
            $task->priority = isset($data->priority) ? $data->priority : 'medium'; // Default if not set
            $task->category = isset($data->category) ? $data->category : 'uncategorized'; // Default if not set
